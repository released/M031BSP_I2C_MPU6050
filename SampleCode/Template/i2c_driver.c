...
uint8_t u8TxAddr = 1;
uint8_t u8RxAddr = 1;
uint8_t u8TxAddr_2bytes = 1;
uint8_t u8RxAddr_2bytes = 1;
...
void I2Cx_Master_LOG(uint8_t indicator , uint32_t u32Status)
{
	#if defined (DEBUG_LOG_MASTER_LV1)
    printf("(%d)%s  : 0x%2x \r\n",indicator, __FUNCTION__ , u32Status);
	#endif
}
